import "./test-finedust.css";

import React from "https://esm.sh/react";
import { createRoot } from "https://esm.sh/react-dom";
function bodyStyling() {
  document.body.style.backgroundColor = `#f4f6f8`;
}
bodyStyling();
/* -------------------------------------------------------------------------- */

const data = {
  weather: "13 좋음",
  dust: "미세먼지",
};
const renderImage = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="36"
    height="36"
    viewBox="0 0 36 36"
    fill="none"
  >
    <path
      d="M18 36C22.7739 36 27.3523 34.1036 30.7279 30.7279C34.1036 27.3523 36 22.7739 36 18C36 13.2261 34.1036 8.64773 30.7279 5.27208C27.3523 1.89642 22.7739 9.29512e-07 18 0L18 18V36Z"
      fill="url(#paint0_linear_2201_34)"
    />
    <path
      d="M18 0C13.2261 -5.69281e-08 8.64773 1.89642 5.27208 5.27208C1.89642 8.64773 4.17347e-07 13.2261 0 18C-4.17347e-07 22.7739 1.89642 27.3523 5.27208 30.7279C8.64773 34.1036 13.2261 36 18 36L18 18V0Z"
      fill="url(#paint1_linear_2201_34)"
    />
    <defs>
      <linearGradient
        id="paint0_linear_2201_34"
        x1="18"
        y1="0"
        x2="18"
        y2="31.0909"
        gradientUnits="userSpaceOnUse"
      >
        <stop stop-color="#5E9EFD" />
        <stop offset="1" stop-color="#B1D0FE" />
      </linearGradient>
      <linearGradient
        id="paint1_linear_2201_34"
        x1="18"
        y1="0"
        x2="18"
        y2="30.2727"
        gradientUnits="userSpaceOnUse"
      >
        <stop stop-color="#BBD6FF" />
        <stop offset="1" stop-color="#E9F2FF" />
      </linearGradient>
    </defs>
  </svg>
);

const renderWeaterWidget = () => (
  <div className="fineDust">
    <div role="group">
      <p className="weather">{data.weather}</p>
      <p className="dust">{data.dust}</p>
    </div>
    <div className="imgContainer">{renderImage()}</div>
  </div>
);

createRoot(document.getElementById("root")).render(renderWeaterWidget());
